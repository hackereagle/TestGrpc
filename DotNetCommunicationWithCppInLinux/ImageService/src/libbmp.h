//This Header is cloned by https://github.com/marc-q/libbmp
...
#define BMP_GET_PADDING(a) (((a + 3) & -4) - a )

typedef void (*ImagePointerCreationCallBack)(void* & , size_t &) ;
typedef void* (*MallocImageBuffer)(int);
...
enum bmp_error	bmp_img_read2				(bmp_img_2*,
												const char *) ;

void bmp_img_2_free(bmp_img_2 *) ;
#if _WIN32
enum bmp_error bmp_img_read_ext(const wchar_t *filename, bmp_header &header , void *& payload , ImagePointerCreationCallBack ) ;

enum bmp_error bmp_img_read_ext_2(const wchar_t *filename, bmp_header &header, void *& payload, MallocImageBuffer);
#endif
enum bmp_error bmp_img_read_ext_3(const char *filename, bmp_header &header, void *& payload, MallocImageBuffer);
...
void            bmp_header_init_df_2            (bmp_header*,
                                                const int,
                                                const int, 
												const int);
#if _WIN32 
